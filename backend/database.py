...
class DebuggerDatabase:
    ...
    def get_last_7_debugs(self, user_id: str) -> list:
        """Get debug session history for a user"""
        conn = sqlite3.connect(self.db_path)
        c = conn.cursor()
    ...
    def create_user(self, email: str, password_hash: str, display_name: str, anonymous_id: str = None) -> int:
        conn = sqlite3.connect(self.db_path)
        c = conn.cursor()
        try:
            c.execute('''
    ...
    def get_user_by_email(self, email: str) -> dict | None:
        conn = sqlite3.connect(self.db_path)
        c = conn.cursor()
        c.execute('SELECT id, email, password_hash, display_name, created_at FROM users WHERE email = ?', (email,))
        row = c.fetchone()
        conn.close()
        if not row:
            return None
    ...
    def migrate_anonymous_data(self, anonymous_id: str, new_user_id: str):
        if not anonymous_id:
            return
        conn = sqlite3.connect(self.db_path)
        c = conn.cursor()
        new_id = str(new_user_id)
        for table in ['debug_history', 'usage_tracking']:
            c.execute(f'UPDATE {table} SET user_id = ? WHERE user_id = ?', (new_id, anonymous_id))
        for table in ['assessments', 'learning_objectives', 'recommendations']:
            c.execute(f'UPDATE {table} SET student_id = ? WHERE student_id = ?', (new_id, anonymous_id))
        conn.commit()
        conn.close()
    ...
    def get_user_by_id(self, user_id: int) -> dict | None:
        conn = sqlite3.connect(self.db_path)
        c = conn.cursor()
        c.execute('SELECT id, email, display_name, created_at FROM users WHERE id = ?', (user_id,))
        row = c.fetchone()
        conn.close()
        if not row:
            return None
