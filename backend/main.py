...
async def login(request: LoginRequest):
    ...
    if request.anonymous_id:
        db.migrate_anonymous_data(request.anonymous_id, str(user['id']))

    token = create_token(user['id'], email)
    return AuthResponse(token=token, user_id=user['id'], email=email, display_name=user['display_name'])
...
async def run_code(request: RunRequest):
    ...
    if lang_lower in ("python", "auto-detect", ""):
        ...
        try:
            ...
            proc.kill()
            proc.communicate()
            return {"output": "", "error": "⏱ Execution timed out (15 second limit)", "exit_code": -1, "language": "Python"}
        except Exception as e:
